...
;( function ( $, window, document, undefined) {
    ...
    Footnote.prototype.init = function(){
        ...
        this.scrollTop          = "";
        // we'll need to know this for screen lock later.
    ...
    Footnote.prototype.lockScreen = function() {
        ...
        // setting html's overflow to hidden' will kill any scrollbars 
        // which will make the page effectively one scrollbar wider.
        // that makes the text jump around. set some padding to counteract
        // note that '.navMenu' is included here. Because it is a fixed-position element,
        // it isn't modified by body. Small, shameful hack.
        $('body, .navMenu').css({'width' : this.windowWidth + 'px', 'margin-right' : this.scrollBarWidth + 'px'});
    ...
    */
    Footnote.prototype.unlockScreen = function() {
        ...
        $('body, .navMenu').css({'width' : 'auto', 'margin-right' : '0px'});
        $(document).off("click.mobileFootnote");
    };
    
    
    Footnote.prototype.measureScrollbar = function () { // thx walsh
        var $scrollDiv = $('<div class="scrollbarMeasure"></div>');
        $('body').append($scrollDiv);
        // innerWidth and outerWidth include the scrollbar in this case.
        // so we need to go manually to $[0].clientWidth to get
        // what should be .innerWidth(). 
        var scrollbarWidth = $scrollDiv.outerWidth() - $scrollDiv[0].clientWidth;
        $scrollDiv.remove();
